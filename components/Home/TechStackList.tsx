...
const Notification = ({ name, description, icon, color, link }: Item) => {
  return (
    ...
    >
      <Link href={link || "/"} target="_blank" className="flex flex-row items-center gap-3">
        <div
...
export default function TechStackList({ items }: { items: Item[] }) {

  let notifications = items;
  notifications = Array.from({ length: 10 }, () => items).flat();
